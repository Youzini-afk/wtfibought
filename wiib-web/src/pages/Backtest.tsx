...
import {
  ...
  CircleSlash, Crosshair, FlaskConical, Gauge, History, Loader2, LogOut, Pause, Play,
...
/** 找 barTimeMs 落在哪根 bar：最后一根 openTime ≤ t 的下标（二分） */
function barIndexAt(bars: number[][], t: number): number {
...
export function Backtest() {
  ...
  const chartCursor = cursor == null ? bars.length : Math.floor(cursor);
  ...
  return (
    <div className="page-shell p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black tracking-tight">可视化回测</h1>
          <p className="text-[11px] text-muted-foreground">本地 5m K线 · 与实盘同一撮合口径 · 右侧工作记录还原策略每一步</p>
        </div>
      </div>
      ...
      <div className="rounded-lg pt-card p-4 md:p-5 space-y-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <div className="flex rounded-md border border-border overflow-hidden">
              {['BTCUSDT', 'ETHUSDT'].map(sym => (
                <button key={sym} type="button" disabled={running || submitting}
          ...
          <div>
            ...
            <input type="date" value={fromDate} disabled={running || submitting}
          ...
          <div>
            ...
            <input type="date" value={toDate} disabled={running || submitting}
              onChange={e => setToDate(e.target.value)}
              className="h-9 px-2.5 rounded-md border border-border bg-input text-xs num disabled:opacity-60" />
          </div>
          <div>
            <div className="microlabel uppercase mb-1">初始资金</div>
            <input type="number" min={1} value={balance} disabled={running || submitting}
          ...
          <div>
            ...
            <input type="number" min={1} max={100} value={leverage} disabled={running || submitting}
          ...
          >
            {running || submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
            {running ? '回测中…' : '跑回测'}
          </button>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
          ...
          <div className="space-y-5 min-w-0">
            <div className="rounded-lg pt-card p-3 md:p-4 space-y-3">
              ...
              <BacktestChart bars={bars} trades={result?.trades ?? []} cursor={chartCursor} />
              ...
              {done && bars.length > 0 && (
                <div className="flex items-center gap-2.5 flex-wrap pt-1 border-t border-border/40">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg border border-border hover:bg-surface-hover flex items-center justify-center text-primary machined"
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-lg border border-border hover:bg-surface-hover flex items-center justify-center text-muted-foreground hover:text-primary"
                    aria-label="复位到全量"
                    title="复位到全量"
                  >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {SPEEDS.map(sp => (
                      <button key={sp.bps} type="button" onClick={() => setSpeed(sp.bps)}
                        className={cn('px-2 h-6 rounded text-[10px] font-bold transition-colors',
