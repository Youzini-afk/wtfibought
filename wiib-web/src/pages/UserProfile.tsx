...
import {
  ArrowLeft, Briefcase, ChevronLeft, ChevronRight, EyeOff, Trophy, Bot,
} from 'lucide-react';
import type { PageResult, ProfilePosition, PublicTrade, UserProfile as UserProfileData } from '../types';
...
export function UserProfile() {
  ...
  // 两处 loading 都由"已加载 key 是否追上请求 key"派生：在 effect 里同步 setLoading(true)
  ...
  if (denied || !profile) {
    return (
      <div className="page-shell p-4 md:p-6 space-y-4">
        ...
        <Card>
          <CardContent className="p-0">
            <EmptyState icon={<EyeOff />} text="该用户没有公开自己的持仓与交易记录" />
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="page-shell p-4 md:p-6 space-y-4">
          ...
          <div className="flex items-start gap-4 flex-wrap">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-2">
                ...
                <div>
                  <div className="microlabel" title="合约+现货+预测">硬实力盈亏</div>
                  <ProfitText value={s.hardcoreProfit} className="text-base font-bold" />
                </div>
                <div>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-black flex items-center gap-2">
            交易历史
            <span className="text-xs text-muted-foreground font-normal">共 {trades.total} 笔</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {tradesLoading ? (
          ...
          ) : trades.records.length === 0 ? (
            <EmptyState icon={<Briefcase />} text="暂无成交记录" />
          ) : (
            <>
              {trades.records.map(t => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => navigate(tradeHref(t.symbol))}
              ...
              {trades.pages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-border/30">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      variant="ghost" size="sm" className="h-8 w-8 p-0"
                      disabled={tradePage >= trades.pages}
                      onClick={() => setTradePage(p => p + 1)}
                    >
